 ...
 * 
 * Usage: npx tsx scripts/make-user-admin.ts test@test.com
 */
...
async function makeUserAdmin(email: string) {
  ...
  try {
    ...
    const result = await client.mutation(api.users.makeUserAdminByEmail, {
      email: email,
    });
...
if (!email) {
  ...
  console.log("\nUsage: npx tsx scripts/make-user-admin.ts <email>");
  console.log("Example: npx tsx scripts/make-user-admin.ts test@test.com");
  process.exit(1);
}
